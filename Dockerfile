...
ARG PHP_EXTS="bcmath ctype fileinfo mbstring pdo pdo_mysql  dom pcntl intl"
ARG PHP_PECL_EXTS="redis mongodb"

# alexo - we need the API key at build time to be able to run artisan route:cache
ARG ARG_APP_KEY
ARG ARG_TEST_DEFAULT="default arg value from Dockerfile"
ARG ARG_TEST_NO_DEFAULT
ARG ARG_TEST_INTERPOLATION
...
RUN mkdir -p /opt/apps/laravel-bunnyshell /opt/apps/laravel-bunnyshell/bin

# Next, set our working directory
WORKDIR /opt/apps/laravel-bunnyshell
...
RUN addgroup -S composer \
    ...
    && chown -R composer /opt/apps/laravel-bunnyshell \
    && apk add --update icu icu-dev \
...
COPY --from=composer_base /opt/apps/laravel-bunnyshell /opt/apps/laravel-bunnyshell
...
COPY --from=composer_base /opt/apps/laravel-bunnyshell /opt/apps/laravel-bunnyshell
COPY --from=frontend /opt/apps/laravel-bunnyshell/public /opt/apps/laravel-bunnyshell/public
...
ARG PHP_PECL_EXTS
ARG ARG_APP_KEY
ARG ARG_TEST_DEFAULT
ARG ARG_TEST

ENV APP_KEY=$ARG_APP_KEY
ENV APP_KEY_2=${ARG_APP_KEY}
ENV ENV_TEST_DEFAULT=$ARG_TEST_DEFAULT
ENV ENV_TEST_NO_DEFAULT=$ARG_TEST_NO_DEFAULT
...
COPY --from=composer_base --chown=www-data /opt/apps/laravel-bunnyshell /opt/apps/laravel-bunnyshell
COPY --from=frontend --chown=www-data /opt/apps/laravel-bunnyshell/public /opt/apps/laravel-bunnyshell/public
...
# IMPORTANT: user must be set before running php artisan !!! 
USER  www-data
...
# tmp disable because APP_KEY might not be available when building the image
...
# and configuration.
COPY docker/bunnyshell/nginx.conf.template /etc/nginx/templates/default.conf.template
...
RUN touch laravel.cron && \
    echo "* * * * * cd /opt/apps/laravel-bunnyshell && php artisan schedule:run" >> laravel.cron && \
    crontab laravel.cron

CMD ["crond", "-l", "2", "-f"]

# default container to run
FROM cli
